...
const {pgp, db, getAllRecipes, saveRecipe, getAllSavedRecipes, unsaveRecipe} = require('../db');
    ...
    db.any('SELECT email FROM users WHERE name = $1', [name]).then((data) => {
            ...
            Promise.all([getAllSavedRecipes(email), getAllRecipes()]).then((values) => {
                if (values[0] === null) {
                    ...
                    const all_owned_recipes = values[0].map(obj => obj.ingredientname);
                    var ret = values[1];
                    ret.forEach(element => element.isSaved = all_owned_recipes.includes(element.ingredientname));
